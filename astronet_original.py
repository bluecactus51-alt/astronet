...
### python astronet.py 225 64 1e-5 '/data/kepler/new' '/data/ensembling/temp'
...
parser.add_argument("--fixed_seed", help="set if wanting to fix the random seed", action="store_true")
...
class AstronetModel(nn.Module):
    ...
    def forward(self, x_local, x_global):
        ...
        ### flattening outputs from convolutional layers 
...
def invert_tensor(tensor):
    ...
    PURPOSE: FLIP A 1D TENSOR ALONG ITS AXIS (FOR DATA AUGMENTATION)
    INPUT: 1D TENSOR
...
def train_model(n_epochs, kepler_data_loader, kepler_val_loader, model, criterion, optimizer):
    ...
    INPUTS:  num_epoch = number of epochs for training
             kepler_data_loader = data loader for training set
             kepler_val_loader = data loader for validation det
             model = model being trained
             criterion = criterion for calculating loss
             optimizer = optimizer used for training

    OUTPUT:  epoch_train_loss = training set loss for each epoch
             epoch_val_loss = validation set loss for each epoch
             epoch_val_acc = validation set accuracy for each epoch
             epoch_val_ap = validation set average precision for each epoch
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for x_train_data, y_train in kepler_data_loader:
            ...
            x_train_local, x_train_global = x_train_data
            ...
            ### randomly invert half of light curves (for data augmentation)
        ...
        for x_val_data, y_val in kepler_val_loader:
            ...
            x_val_local, x_val_global = x_val_data
            ...
            ### (used for avg. precision per epoch; reset at each epoch; final values output)
...
### save plot
plotname = 'r' + str(run).zfill(2) + '-i' + str(n_epochs) + '-plot.pdf'
plt.savefig(os.path.join(args.m_out, plotname), bbox_inches='tight', dpi=200, rastersized=True, alpha=True)
